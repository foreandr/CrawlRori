...
def create_ruimtes_tab(tab, ruimtes_data):
    """
    Creates the Ruimtes tab content with dynamic tabs for each ruimte, adding pagination to limit the number of visible tabs.
    """
    if "data" in ruimtes_data and isinstance(ruimtes_data["data"], list):
        # Pagination variables
        items_per_page = 5  # Number of tabs to display per page
        total_items = len(ruimtes_data["data"])
        ...
        # Create the main TabView for Ruimtes
        ...
        #print("main_tabview initialized for Ruimtes.")
        ...
        def load_page(page):
            ...
            for i in range(start_idx, end_idx):
                ruimte_data = ruimtes_data["data"][i]
                ruimte_title = ruimte_data.get("title", f"Ruimte {i + 1}")
                tabs_list.append(ruimte_title)  # Track the tab name
                main_tabview.add(ruimte_title)
        ...
        def lazy_load_tab_content():
            ...
            # Find the corresponding ruimte data by title
            for ruimte_data in ruimtes_data["data"]:
                if ruimte_data.get("title", "Unknown Ruimte") == selected_tab:
                    ruimte_tab = main_tabview.tab(selected_tab)
